...
$hosts_aceptados = array('localhost','127.0.0.1', '192.168.167.21');
...
if(in_array($_SERVER['HTTP_HOST'],$hosts_aceptados)){
        ...
        if(isset($txt_usuario)&& !empty($txt_usuario)){
            ...
            if(isset($txt_password)&& !empty($txt_password)){
                ...
                if($txt_usuario == $usuario_correcto){
                    ...
                    $ruta = "";
                    $msg = "El usuario no esta permitido"; 
                    $codigo_estado = 401;
                    $texto_estado = "Unauthorized";
                    $token = "";
                }
    ...
    $ruta = "";
    $msg = "La direccion IP no esta permitida"; 
    $codigo_estado = 406;
...
$arreglo_respuesta =array(
    "status"=>((inval($codigo_estado)==200) ? "succes": "error"),
    "error" =>((inval($codigo_estado)==200) ? "" : array("code"=>$codigo_estado,"message"=>$msg)),
    "data" =>array(
        "url"=>$ruta,
        "token"=>$token
    ),
